...
for entry in \
  'RK1UO:G1C:4/3/280' \
  'RK1ULK:RK1UO:2/1/790' \
  'RK1ULA:RK1ULK:4/2/690' \
  'RK1P:RK1ULA:8/5/800' \
  'BA1A:G1C:2/1/250' \
  'BA1SF1:BA1A:5/3/760' \
...
if ! rg -q 'RK1UO[[:space:]]*->[[:space:]]*RK1ULK[[:space:]]*->[[:space:]]*RK1ULA[[:space:]]*->[[:space:]]*RK1P' "$plan"; then
  fail 'runner serialization must order RK1UO -> RK1ULK -> RK1ULA -> RK1P'
fi
...
expect_count "$catalog" 47 'fixed catalog count'
expect_count "$((catalog - 2))" 45 'remaining incomplete slice count'
expect_count "$gates" 10 'release gate count'
if ! rg -Fq '19 safety slices remain' "$plan" "$todo"; then
  fail 'roadmap must state exactly: 19 safety slices remain'
fi
...
case $todo_active in *G3*RK1ULK*BA1SF1*RK1UO*BA1A*) ;; *) fail 'active todo milestones must name G3, the active pair, and both closed foundations' ;; esac
...
for stale in \
  ...
  '| RK1UL |' '| 1B | RK1UL + BA1SF1 |' 'RK1UO -> RK1UL -> RK1P' \
