...
// ReadDirectorySnapshotFile returns one regular file captured below a complete
// recursive directory authority. rel must already be a clean slash-relative
// path. Returned bytes never alias the opaque snapshot.
func ReadDirectorySnapshotFile(snapshot *DirectorySnapshot, rel string) ([]byte, fs.FileMode, error) {
	...
	return append([]byte(nil), entry.data...), entry.mode.Perm(), nil
}
...
func SubdirectorySnapshot(snapshot *DirectorySnapshot, rel string) (*DirectorySnapshot, error) {
	...
	return newDirectorySnapshot(cloneDirectorySnapshotNode(*entry.dir)), nil
}
...
func cloneDirectorySnapshotNode(source directorySnapshotNode) directorySnapshotNode {
	...
	for index, entry := range source.entries {
		clone.entries[index] = directorySnapshotEntry{name: entry.name, mode: entry.mode, data: append([]byte(nil), entry.data...)}
		if entry.dir != nil {
