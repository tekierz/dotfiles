...
func TestBuildInstallPlanContainsCoreInstallsAndExactConfigActions(t *testing.T) {
	...
	for _, id := range []string{"tmux", "ghostty", "zsh", "neovim", "git", "fzf"} {
	...
	for _, action := range plan.actions() {
		if action.ID == "config:yazi" {
			t.Fatalf("core plan retained aggregate Yazi action: %+v", action)
		}
	}
	for _, id := range []string{"config:yazi:main", "config:yazi:keymap", "config:yazi:theme"} {
		action := planActionByID(t, plan, id)
		if action.Disposition != operation.DispositionApply || action.ToolID != "yazi" {
			t.Fatalf("core split Yazi action %s = %+v", id, action)
		}
	}
	configToolCounts := make(map[string]int, len(plan.configTools))
	for _, toolID := range plan.configTools {
		configToolCounts[toolID]++
	}
	if got := configToolCounts["yazi"]; got != 1 {
		t.Fatalf("logical Yazi installer dispatch count = %d in %v, want 1 despite split config actions", got, plan.configTools)
	}
	for toolID, count := range configToolCounts {
		if toolID != "yazi" && count > 1 {
			t.Errorf("logical %s installer dispatch count = %d in %v, want at most 1", toolID, count, plan.configTools)
		}
	}
	for _, action := range plan.actions() {
...
func TestBuildInstallPlanSplitsYaziMainAtResolvedXDGPath(t *testing.T) {
	app, home, runtime := newPlanTestApp(t)
	xdg := filepath.Join(home, "reviewed-xdg")
	t.Setenv("XDG_CONFIG_HOME", xdg)
	...
	var mainAction operation.Action
	foundMain := false
	for _, action := range plan.actions() {
		if action.ID == "config:yazi" {
			t.Fatalf("legacy aggregate Yazi action remains in plan: %+v", action)
		}
		if action.ID == "config:yazi:main" {
			mainAction = action
			foundMain = true
		}
	}
	if !foundMain {
		t.Fatal("split Yazi main action not found")
	}
	mainPath := filepath.Join(xdg, "yazi", tools.YaziFileMain)
	wantTarget := planTargetPath(home, mainPath)
	if mainAction.Disposition != operation.DispositionApply || mainAction.ToolID != "yazi" || mainAction.Ownership != operation.OwnershipManagedFile {
		t.Fatalf("Yazi main action metadata=%+v", mainAction)
	}
	if mainAction.Target != wantTarget || !slices.Equal(mainAction.BackupTargets, []string{wantTarget}) || len(mainAction.Observations) != 1 {
		t.Fatalf("Yazi main action targets/observations=%+v", mainAction)
	}
	observation := mainAction.Observations[0]
	if observation.Source != wantTarget || observation.Exists || observation.Managed || mainAction.Observation.Exists || mainAction.Observation.Managed {
		t.Fatalf("Yazi main missing observation=%+v combined=%+v", observation, mainAction.Observation)
	}
	authority := plan.authority[mainAction.ID]
	...
	if len(authority) != 1 || !ok || accepted.kind != acceptedFileTarget || !accepted.file.Tracked() || accepted.file.Exists() || !accepted.parents.Tracked() {
		t.Fatalf("Yazi main accepted authority=%+v map=%+v", accepted, authority)
	}
	if _, statErr := os.Lstat(mainPath); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("plan preview mutated Yazi main path: %v", statErr)
	}
	for _, path := range []string{filepath.Dir(mainPath), filepath.Join(home, ".config", "yazi")} {
		if _, statErr := os.Lstat(path); !errors.Is(statErr, os.ErrNotExist) {
			t.Fatalf("plan preview created inactive Yazi directory %s: %v", path, statErr)
		}
	}
	backupCount := 0
	for _, target := range plan.backupTargets() {
	...
	if backupCount != 1 {
		t.Fatalf("Yazi main backup target count=%d in %v, want 1", backupCount, plan.backupTargets())
	}
}

func TestBuildInstallPlanSplitsYaziKeymapIndependentlyAtResolvedXDGPath(t *testing.T) {
	app, home, runtime := newPlanTestApp(t)
	xdg := filepath.Join(home, "reviewed-xdg")
	t.Setenv("XDG_CONFIG_HOME", xdg)
	t.Setenv("YAZI_CONFIG_HOME", "")
	dir := filepath.Join(xdg, "yazi")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		t.Fatal(err)
	}
	mainPath := filepath.Join(dir, tools.YaziFileMain)
	themePath := filepath.Join(dir, tools.YaziFileTheme)
	keymapPath := filepath.Join(dir, tools.YaziFileKeymap)
	mainSentinel := []byte("[mgr]\nshow_hidden = true\n")
	themeSentinel := []byte("[mgr\ncwd = {}\n")
	mainInspection := tools.InspectYaziConfigContent(tools.YaziFileKindMain, mainPath, mainSentinel, true)
	if mainInspection.Ownership != tools.YaziOwnershipNative || mainInspection.ReadOnlyReason == "" {
		t.Fatalf("main sentinel classification=%+v, want native read-only", mainInspection)
	}
	themeInspection := tools.InspectYaziConfigContent(tools.YaziFileKindTheme, themePath, themeSentinel, true)
	if themeInspection.Ownership != tools.YaziOwnershipMalformed || themeInspection.Error == "" || themeInspection.ReadOnlyReason == "" {
		t.Fatalf("theme sentinel classification=%+v, want malformed error/read-only", themeInspection)
	}
	if err := os.WriteFile(mainPath, mainSentinel, 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(themePath, themeSentinel, 0o600); err != nil {
		t.Fatal(err)
	}
	readRevision := func(path string) ([]byte, safefile.Revision) {
		t.Helper()
	...
	_, mainRevision := readRevision(mainPath)
	_, themeRevision := readRevision(themePath)
	...
	for _, candidate := range plan.actions() {
		if candidate.ID == "config:yazi:main" {
			mainAction = candidate
			foundMain = true
		}
		if candidate.ID == "config:yazi:keymap" {
			action = candidate
			found = true
		}
	}
	if !foundMain || mainAction.Disposition != operation.DispositionBlocked || !strings.Contains(strings.ToLower(mainAction.Reason), tools.YaziFileMain) || !strings.Contains(strings.ToLower(mainAction.Reason), "native") {
		t.Fatalf("hostile main split action=%+v found=%t, want blocked native reason", mainAction, foundMain)
	}
	if _, ok := plan.authority[mainAction.ID]; ok {
		t.Fatalf("blocked Yazi main action gained authority: %+v", plan.authority[mainAction.ID])
	}
	if !found {
		t.Fatal("split Yazi keymap action not found")
	}
	wantTarget := planTargetPath(home, keymapPath)
	if action.Disposition != operation.DispositionApply || action.ToolID != "yazi" || action.Ownership != operation.OwnershipManagedFile || action.Target != wantTarget {
		t.Fatalf("Yazi keymap action metadata=%+v", action)
	}
	if !slices.Equal(action.BackupTargets, []string{wantTarget}) || len(action.Observations) != 1 || action.Observations[0].Source != wantTarget || action.Observations[0].Exists || action.Observations[0].Managed {
		t.Fatalf("Yazi keymap action targets/observations=%+v", action)
	}
	authority := plan.authority[action.ID]
	accepted, ok := authority[wantTarget]
	if len(authority) != 1 || !ok || accepted.kind != acceptedFileTarget || !accepted.file.Tracked() || accepted.file.Exists() || !accepted.parents.Tracked() {
		t.Fatalf("Yazi keymap accepted authority=%+v map=%+v", accepted, authority)
	}
	backupCount := 0
	for _, target := range plan.backupTargets() {
		if target == wantTarget {
	...
	if backupCount != 1 {
		t.Fatalf("Yazi keymap backup target count=%d in %v, want 1", backupCount, plan.backupTargets())
	}
	for path, before := range map[string]struct {
		data     []byte
		revision safefile.Revision
	}{mainPath: {mainSentinel, mainRevision}, themePath: {themeSentinel, themeRevision}} {
		data, revision := readRevision(path)
		if revision != before.revision || !bytes.Equal(data, before.data) {
			t.Fatalf("Yazi sibling %s changed: revision=%+v want=%+v data=%q", path, revision, before.revision, data)
		}
	}
	if _, statErr := os.Lstat(keymapPath); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("plan preview mutated Yazi keymap path: %v", statErr)
	}
}

func TestBuildInstallPlanBlocksLegacyHeaderYaziThemeIndependently(t *testing.T) {
	app, home, runtime := newPlanTestApp(t)
	xdg := filepath.Join(home, "reviewed-xdg")
	t.Setenv("XDG_CONFIG_HOME", xdg)
	t.Setenv("YAZI_CONFIG_HOME", "")
	...
	themePath := filepath.Join(dir, tools.YaziFileTheme)
	themeBytes := []byte("# Theme: dracula (generated by dotfiles)\n[mgr]\ncwd = {}\n")
	inspection := tools.InspectYaziConfigContent(tools.YaziFileKindTheme, themePath, themeBytes, true)
	if inspection.Ownership != tools.YaziOwnershipNative || inspection.ReadOnlyReason == "" || inspection.Error != "" {
		t.Fatalf("legacy-header theme classification=%+v, want native read-only without parse error", inspection)
	}
	if err := os.WriteFile(themePath, themeBytes, 0o600); err != nil {
		t.Fatal(err)
	}
	themeRel, err := filepath.Rel(home, themePath)
	if err != nil {
		t.Fatal(err)
	}
	_, themeRevision, err := safefile.ReadWithin(home, filepath.ToSlash(themeRel))
	if err != nil {
		t.Fatal(err)
	}
	plan, err := buildInstallPlan(app, runtime, time.Now())
	...
	var mainAction, keymapAction, themeAction operation.Action
	foundMain, foundKeymap, foundTheme := false, false, false
	for _, action := range plan.actions() {
		switch action.ID {
		case "config:yazi":
			t.Fatalf("legacy aggregate Yazi action remains in plan: %+v", action)
		...
		case "config:yazi:theme":
			themeAction, foundTheme = action, true
		}
	}
	for name, state := range map[string]struct {
		found  bool
		action operation.Action
	}{"main": {foundMain, mainAction}, "keymap": {foundKeymap, keymapAction}} {
		if !state.found || state.action.Disposition != operation.DispositionApply || len(plan.authority[state.action.ID]) != 1 {
			t.Fatalf("independent Yazi %s action=%+v found=%t authority=%+v", name, state.action, state.found, plan.authority[state.action.ID])
		}
	}
	if !foundTheme {
		t.Fatal("split Yazi theme action not found")
	}
	wantTarget := planTargetPath(home, themePath)
	if themeAction.Disposition != operation.DispositionBlocked || themeAction.Target != wantTarget || themeAction.ToolID != "yazi" || themeAction.Ownership != operation.OwnershipManagedFile {
		t.Fatalf("legacy-header Yazi theme action=%+v", themeAction)
	}
	if len(themeAction.Observations) != 1 || themeAction.Observations[0].Source != wantTarget || !themeAction.Observations[0].Exists || themeAction.Observations[0].Managed {
		t.Fatalf("legacy-header Yazi theme observation=%+v", themeAction.Observations)
	}
	reason := strings.ToLower(themeAction.Reason)
	if !strings.Contains(reason, tools.YaziFileTheme) || !strings.Contains(reason, "native") {
		t.Fatalf("legacy-header Yazi theme reason=%q", themeAction.Reason)
	}
	if _, ok := plan.authority[themeAction.ID]; ok {
		t.Fatalf("blocked legacy-header theme gained authority: %+v", plan.authority[themeAction.ID])
	}
	if slices.Contains(plan.backupTargets(), wantTarget) {
		t.Fatalf("blocked legacy-header theme leaked into plan backups: %v", plan.backupTargets())
	}
	got, current, err := safefile.ReadWithin(home, filepath.ToSlash(themeRel))
	if err != nil || current != themeRevision || !bytes.Equal(got, themeBytes) {
		t.Fatalf("legacy-header theme changed: revision=%+v want=%+v bytes=%q err=%v", current, themeRevision, got, err)
	}
}

func TestBuildInstallPlanAppliesMissingYaziThemeAtFrozenOverride(t *testing.T) {
	app, home, runtime := newPlanTestApp(t)
	...
	if plan.yaziConfigPaths.Origin != tools.YaziConfigOriginOverride || plan.yaziConfigPaths.Dir != override || plan.yaziConfigPaths.Theme != themePath {
		t.Fatalf("frozen Yazi override paths=%+v, want dir=%s theme=%s", plan.yaziConfigPaths, override, themePath)
	}
	var action operation.Action
	found := false
	for _, candidate := range plan.actions() {
		if candidate.ID == "config:yazi:theme" {
			action = candidate
			found = true
		}
	}
	...
	wantTarget := planTargetPath(home, themePath)
	if action.Disposition != operation.DispositionApply || action.ToolID != "yazi" || action.Ownership != operation.OwnershipManagedFile || action.Target != wantTarget {
		t.Fatalf("missing Yazi theme action=%+v", action)
	}
	if !slices.Equal(action.BackupTargets, []string{wantTarget}) || len(action.Observations) != 1 || action.Observations[0].Source != wantTarget || action.Observations[0].Exists || action.Observations[0].Managed {
		t.Fatalf("missing Yazi theme targets/observation=%+v", action)
	}
	authority := plan.authority[action.ID]
	accepted, ok := authority[wantTarget]
	if len(authority) != 1 || !ok || accepted.kind != acceptedFileTarget || !accepted.file.Tracked() || accepted.file.Exists() || !accepted.parents.Tracked() {
		t.Fatalf("missing Yazi theme authority=%+v map=%+v", accepted, authority)
	}
	backupCount := 0
	for _, target := range plan.backupTargets() {
		if target == wantTarget {
	...
	if backupCount != 1 {
		t.Fatalf("Yazi theme backup target count=%d in %v, want 1", backupCount, plan.backupTargets())
	}
	for _, path := range []string{themePath, override} {
		if _, statErr := os.Lstat(path); !errors.Is(statErr, os.ErrNotExist) {
			t.Fatalf("plan preview created Yazi override path %s: %v", path, statErr)
		}
	}
}
...
func TestInstallPlanWorkerUsesFrozenYaziAndGlobalPathsAfterEnvironmentDrift(t *testing.T) {
	app, home, _ := newPlanTestApp(t)
	stateA := filepath.Join(home, "state-a")
	yaziA := filepath.Join(home, "yazi-a")
	t.Setenv("XDG_CONFIG_HOME", stateA)
	t.Setenv("YAZI_CONFIG_HOME", yaziA)
	...
	runtime := registryRuntime(pkg.PlatformMacOS, app.manageInstalled)
	runtime.backupTargets = func(targets []backup.Target) (autoBackupResult, error) {
		backupDir := filepath.Join(home, "reviewed-backups", "frozen-install")
		result, err := backup.CreatePlanTracked(home, backupDir, targets)
		return autoBackupResult{enabled: true, count: result.Count, backupDir: backupDir, plan: &result}, err
	}
	plan, err := buildInstallPlan(app, runtime, time.Now())
	if err != nil || plan.hasBlocked() {
		t.Fatalf("frozen install plan blocked=%v err=%v", plan != nil && plan.hasBlocked(), err)
	}
	globalPathA := filepath.Join(stateA, "dotfiles", "global.json")
	globalTarget := planTargetPath(home, globalPathA)
	globalAction := planActionByID(t, plan, "state:global")
	if globalAction.Target != globalTarget || globalAction.BackupTarget != globalTarget || len(globalAction.Observations) != 1 || globalAction.Observations[0].Source != globalTarget {
		t.Fatalf("frozen installer global action=%+v want target=%s", globalAction, globalTarget)
	}
	globalAuthority := plan.authority[globalAction.ID]
	acceptedGlobal, ok := globalAuthority[globalTarget]
	if len(globalAuthority) != 1 || !ok || acceptedGlobal.kind != acceptedFileTarget || !acceptedGlobal.file.Tracked() || !acceptedGlobal.parents.Tracked() {
		t.Fatalf("frozen installer global authority=%+v map=%+v", acceptedGlobal, globalAuthority)
	}
	plannedGlobalRevision, tracked := config.GlobalConfigRevision(plan.globalConfig)
	if !tracked || plannedGlobalRevision != acceptedGlobal.file {
		t.Fatalf("planned global revision=%+v tracked=%t accepted=%+v", plannedGlobalRevision, tracked, acceptedGlobal.file)
	}
	stateB := filepath.Join(home, "state-b")
	yaziB := filepath.Join(home, "yazi-b")
	t.Setenv("XDG_CONFIG_HOME", stateB)
	t.Setenv("YAZI_CONFIG_HOME", yaziB)
	events := make(chan installEventMsg, 128)
	go runInstallPlanWorker(context.Background(), events, plan, runtime)
	var terminal installEventMsg
	for event := range events {
	...
	if terminal.err != nil {
		t.Fatalf("frozen install worker failed: %v\n%s", terminal.err, terminal.context)
	}
	yaziCfg := yaziConfigFrom(plan.config)
	wantYazi := map[string][]byte{
		filepath.Join(yaziA, tools.YaziFileMain):   []byte(tools.GenerateYaziConfig(yaziCfg, plan.theme)),
		filepath.Join(yaziA, tools.YaziFileKeymap): []byte(tools.GenerateYaziKeymap(yaziCfg, plan.theme)),
		filepath.Join(yaziA, tools.YaziFileTheme):  []byte(tools.GenerateYaziTheme(plan.theme)),
	}
	for path, want := range wantYazi {
		got, readErr := os.ReadFile(path)
		if readErr != nil || !bytes.Equal(got, want) {
			t.Fatalf("frozen installer Yazi target %s data=%q err=%v want=%q", path, got, readErr, want)
		}
	}
	data, err := os.ReadFile(globalPathA)
	if err != nil {
		t.Fatal(err)
	}
	...
	if saved.SchemaVersion != plan.globalConfig.SchemaVersion || saved.Theme != plan.globalConfig.Theme || saved.NavStyle != plan.globalConfig.NavStyle || saved.ActiveUser != plan.globalConfig.ActiveUser || saved.DisableAnimations != plan.globalConfig.DisableAnimations || saved.AutoBackup != plan.globalConfig.AutoBackup || saved.BackupMaxCount != plan.globalConfig.BackupMaxCount || saved.BackupMaxAgeDays != plan.globalConfig.BackupMaxAgeDays {
		t.Fatalf("frozen installer global=%+v want=%+v", saved, plan.globalConfig)
	}
	for _, path := range []string{
		stateB, filepath.Join(stateB, "dotfiles"), filepath.Join(stateB, "dotfiles", "global.json"),
		yaziB, filepath.Join(yaziB, tools.YaziFileMain), filepath.Join(yaziB, tools.YaziFileKeymap), filepath.Join(yaziB, tools.YaziFileTheme),
	} {
		if _, statErr := os.Lstat(path); !errors.Is(statErr, os.ErrNotExist) {
			t.Fatalf("install environment drift created %s: %v", path, statErr)
		}
	}
}
...
func TestPartialWriterEvidenceCannotAuthorizeEditBeforeWorkerHandling(t *testing.T) {
	...
	rel := planTargetPath(home, plan.yaziConfigPaths.Main)
	...
	if err := recordFailedActionRollbackState(home, plan, actionID, partial, expected); err == nil || !strings.Contains(err.Error(), "manual recovery required") {
		t.Fatalf("untracked partial evidence error = %v, want manual recovery requirement", err)
	}
	state := expected[rel]
	if len(expected) != 1 || !state.Attempted || state.Captured {
		t.Fatalf("external edit gained rollback authority: %+v", state)
	}
	for _, sibling := range []string{planTargetPath(home, plan.yaziConfigPaths.Keymap), planTargetPath(home, plan.yaziConfigPaths.Theme)} {
		if _, attempted := expected[sibling]; attempted {
			t.Fatalf("unattempted Yazi sibling %s gained rollback state: %+v", sibling, expected[sibling])
		}
	}
}
...
func TestMutationEvidenceSetsRejectOutOfScopePathsAtomicallyInBothOrders(t *testing.T) {
	...
	actionID := "config:yazi:main"
	validRel := planTargetPath(home, plan.yaziConfigPaths.Main)
	...
	for _, test := range tests {
		for _, invalidFirst := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				if err := recordFailedActionRollbackState(home, plan, actionID, partial, partialExpected); err == nil || !strings.Contains(err.Error(), "manual recovery required") {
					t.Fatalf("mixed-scope partial evidence error = %v, want manual recovery requirement", err)
				}
				state := partialExpected[validRel]
				if len(partialExpected) != 1 || !state.Attempted || state.Captured {
					t.Fatalf("%s partial evidence gained rollback authority: %+v", validRel, state)
				}
				for _, sibling := range []string{planTargetPath(home, plan.yaziConfigPaths.Keymap), planTargetPath(home, plan.yaziConfigPaths.Theme)} {
					if _, attempted := partialExpected[sibling]; attempted {
						t.Fatalf("unattempted Yazi sibling %s gained rollback state: %+v", sibling, partialExpected[sibling])
					}
				}
			})
		}
