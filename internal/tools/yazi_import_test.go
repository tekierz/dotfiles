...
func TestInspectYaziConfigContentClassifiesStableBytesWithoutEnvironmentReads(t *testing.T) {
	t.Setenv("HOME", "relative-hostile-home")
	t.Setenv("XDG_CONFIG_HOME", "relative-hostile-xdg")
	t.Setenv("YAZI_CONFIG_HOME", "relative-hostile-yazi")
	nonexistentDir := filepath.Join(t.TempDir(), "inspection-must-not-create")
	mainPath := filepath.Join(nonexistentDir, YaziFileMain)
	keymapPath := filepath.Join(nonexistentDir, YaziFileKeymap)
	themePath := filepath.Join(nonexistentDir, YaziFileTheme)
	unknownPath := filepath.Join(nonexistentDir, "unknown.toml")
	cfg := YaziConfig{Keymap: "vim", PreviewMode: "auto", SortBy: "natural", LineMode: "none", ScrollOff: 5}
	...
	tests := []struct {
		...
		{"missing ignores bytes", YaziFileKindMain, mainPath, []byte("[mgr\nmalformed = true\n"), false, YaziOwnershipMissing, false, false},
		{"main exact current", YaziFileKindMain, mainPath, []byte(GenerateYaziConfig(cfg, "nord")), true, YaziOwnershipExactCurrent, false, false},
		{"main native", YaziFileKindMain, mainPath, []byte("[mgr]\nshow_hidden = true\n"), true, YaziOwnershipNative, false, true},
		{"main historical", YaziFileKindMain, mainPath, []byte(generateHistoricalFullYaziConfig(historicalCfg, "nord")), true, YaziOwnershipExactHistorical, false, true},
		{"main malformed", YaziFileKindMain, mainPath, []byte("[mgr\nshow_hidden = true\n"), true, YaziOwnershipMalformed, true, true},
		{"unknown kind", YaziFileKind("unknown"), unknownPath, []byte("value = true\n"), true, YaziOwnershipMalformed, true, true},
		{"keymap exact current", YaziFileKindKeymap, keymapPath, []byte(GenerateYaziKeymap(cfg, "nord")), true, YaziOwnershipExactCurrent, false, false},
		{"keymap native", YaziFileKindKeymap, keymapPath, []byte("[mgr]\nkeymap = []\n"), true, YaziOwnershipNative, false, true},
		{"keymap historical", YaziFileKindKeymap, keymapPath, []byte(generateHistoricalFullYaziKeymap(historicalCfg, "nord")), true, YaziOwnershipExactHistorical, false, true},
		{"keymap malformed", YaziFileKindKeymap, keymapPath, []byte("[mgr\nkeymap = []\n"), true, YaziOwnershipMalformed, true, true},
		{"theme exact current", YaziFileKindTheme, themePath, []byte(GenerateYaziTheme("nord")), true, YaziOwnershipExactCurrent, false, false},
		{"theme native", YaziFileKindTheme, themePath, []byte("[mgr]\ncwd = { fg = \"#ffffff\" }\n"), true, YaziOwnershipNative, false, true},
		{"theme historical", YaziFileKindTheme, themePath, []byte(generateHistoricalFullYaziTheme("nord")), true, YaziOwnershipExactHistorical, false, true},
		{"theme malformed", YaziFileKindTheme, themePath, []byte("[mgr\ncwd = {}\n"), true, YaziOwnershipMalformed, true, true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got := InspectYaziConfigContent(test.kind, test.path, test.content, test.exists)
			if got.Kind != test.kind || got.Path != test.path || got.Exists != test.exists || got.Ownership != test.ownership || got.External {
				t.Fatalf("observation=%#v, want kind=%s path=%s exists=%t ownership=%s", got, test.kind, test.path, test.exists, test.ownership)
			}
			if (got.Error != "") != test.wantError {
				t.Fatalf("observation error=%q, wantError=%t", got.Error, test.wantError)
			}
			if (got.ReadOnlyReason != "") != test.readOnly {
				t.Fatalf("read-only reason=%q, want readOnly=%t", got.ReadOnlyReason, test.readOnly)
			}
		})
	}
	for _, path := range []string{mainPath, keymapPath, themePath, unknownPath} {
		if _, err := os.Lstat(path); !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("pure inspection created %s: %v", path, err)
		}
	}
}
...
func TestImportYaziRecognizesFrozenCurrentOldFullFormatsAsHistorical(t *testing.T) {
	dir := prepareYaziImportDir(t)
	cfg := YaziConfig{Keymap: "emacs", ShowHidden: true, PreviewMode: "never", SortBy: "modified", SortReverse: true, LineMode: "permissions", ScrollOff: 9}
	fixtures := map[string]struct{ content, hash string }{
		YaziFileMain:   {generateHistoricalFullYaziConfig(cfg, "nord"), "01e66e5b6e4ddeff37994f0b40fcbbe7afd687f4e4b8c1ab2d51c90b261566f7"},
		YaziFileKeymap: {generateHistoricalFullYaziKeymap(cfg, "nord"), "9ae84580ff17fc8644d4d315da7bd9a3db46c331fcea0ffd3c03eca72bf03d6e"},
		YaziFileTheme:  {generateHistoricalFullYaziTheme("nord"), "6640ab1fc2322ae40b5d422721f745adbb7a7e4a7b4dcde8c023ffa726117e27"},
	}
	for name, fixture := range fixtures {
		if actual := fmt.Sprintf("%x", sha256.Sum256([]byte(fixture.content))); actual != fixture.hash {
			t.Fatalf("frozen current-old %s bytes drifted: sha256=%s", name, actual)
		}
		writeYaziTestFile(t, filepath.Join(dir, name), fixture.content)
	}
	got, err := ImportYaziConfig()
