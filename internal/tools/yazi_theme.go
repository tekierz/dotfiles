...
func GenerateYaziTheme(theme string) string {
	if _, known := yaziThemePalettes[theme]; !known {
		theme = "catppuccin-mocha"
	}
	return generateCurrentYaziTheme(theme)
}

// generateHistoricalFullYaziTheme reproduces the former full theme bytes for
// exact historical recognition only.
func generateHistoricalFullYaziTheme(theme string) string {
	p := yaziPaletteForTheme(theme)
	return fmt.Sprintf(`# Generated by dotfiles TUI
# Theme: %s
