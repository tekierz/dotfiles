...
Status: **execution active; G3 reconciliation in progress; two foundations closed branch-locally**
...
flowchart TD
  ...
  RK1UO --> RK1UL["RK1UL Ordinary streaming lifecycle"]
  RK1UL --> RK1P["RK1P Privileged supervisor"]
...
| G3 | Replace the rejected runner/restore parents with their frozen serialized slices and reconcile all roadmap accounting | this plan, tasks/todo.md, release-plan test | G1C |
...
The fixed execution catalog contains 46 slices: G2, 20 safety slices, 20 Wave 2 engineering
slices, and five external slices. RK1UO and BA1A are closed branch-locally but not integrated,
so 44 catalog slices remain incomplete and 18 safety slices remain.
...
| RK1UL | Make ordinary streaming cancellation, overflow, scanner failure, and undrained output terminal and bounded around RK1UO | internal/runner/bash.go, exact_streaming.go, lifecycle helper and tests | RK1UO |
| RK1P | Add a trusted exact-argv privileged supervisor that terminates and reaps sudo-owned descendant groups | runner supervisor, narrow CLI dispatch, platform files and tests | RK1UL |
...
The runner chain deliberately separates native observation, ordinary lifecycle, and privileged
supervision. AP1 does not raise channel capacity or discard apt output.
...
| 1B | RK1UL + BA1SF1 | Ordinary lifecycle and restore-parent authority |
| 1C | RK1P + BA1SF2 | Privileged supervisor and exact restore leaves |
| 1D | RS1 + BA1B2 | Sequential streaming and immutable catalog kernel |
| 1E | UR1 + BA1C | Provider domain and opaque restore authority |
| 1F | AP1 + BC1 | Debian adapter and CLI restore |
| 1G | UR2 + BC2 | TUI update routing and TUI restore |
| 1H | CX1 + BU1 | Cancellation propagation and uninstall recovery |
| 1I | BR1 + SH1 | Bounded-read policy and embedded helper |
| 1J | BR2 + BR3 | Config and backup input limits |
...
- internal/runner lifecycle: RK1UO -> RK1UL -> RK1P -> RS1.
...
Requires all 20 safety slices: RK1UO, RK1UL, RK1P, RS1, AP1, UR1-UR2, CX1, BA1A,
...
|------|---------------|-----------|
| RG0 Plan freeze | G0, G1C, and G3 committed; fixed 46-slice catalog; no stale active state | Implementation may begin |
